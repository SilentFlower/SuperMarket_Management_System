...
<div class="container-fluid" id="mid">
    ...
    <div id="main" class="col-md-10" style="height: 100%;">
        ...
        <div id="right-top" style="height: 8%">
            <div id="mianbao">
                <div id="url_name">
                    <h4 style="color: #2aabd2">库存盘点</h4>
                </div>
                <div id="url_name2">
                    <ul class="breadcrumb">
                        <li><a href="${pageContext.request.contextPath}/utils/goHome">首页</a></li>
                        <li class="active">库存管理</li>
                        <li class="active"><a href="${pageContext.request.contextPath}/turnover/get_Turnover">库存盘点
...
<div style="display: none">
    <form action="${pageContext.request.contextPath}/turnover/get_Turnover" method="post" id="submit" >
        <input type="text" value="" name="turnover">
        <input type="text" value="${page.pageSize}" name="pageSize">
        <input type="text" value="${page.currentPage}" name="currentPage">
    </form>
</div>
<input type="hidden" value="" name="g_id" id="g_id">
...
<script>
    ...
    function get_goods(){
        ...
        for(i in allGoods){
            names +='<option value="'+allGoods[i].g_id+'">'+allGoods[i].goods_name+'</option>'
        }
        list_goods_name();
        $("#list_goods_name").html(names);
    }

    function list_goods_name(){
    ...
    function get_key(){
        ...
        if(key != null){
            if(key["g_id"] != null){
                allGoods = ${allGoods};
                for(i in allGoods){
                    if(key["g_id"] == allGoods[i].g_id){
                        $("#g_id").prop("value",key["g_id"]);
                        $("#search_goods_name").prop("value", allGoods[i].goods_name);
                        break;
                    }
                }
            }
    ...
    $("body").on('click','#search_button',function () {
        var turnover = {};
        turnover["g_id"] = $("#g_id").val();
        turnover["startSearchTime"] = $("#search_goods_start").val();
        turnover["endSearchTime"] = $("#search_goods_end").val();
        $("#submit input").eq(0).prop("value", JSON.stringify(turnover));
        submit();
    });

    $("#search_goods_name").change(function () {
        var g_id = $(this).val();
        $("#g_id").prop("value", g_id);
        var allGoods = ${allGoods};
        for(i in allGoods){
            if(allGoods[i].g_id == g_id){
                $(this).prop("value", allGoods[i].goods_name);
                break;
            }
        }
    });
